  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, errorType) {
      ...
      const messages = {
        ...
        minlength:
          label + ' must be at least ' + field.getAttribute('minlength') + ' characters.',
      };
    ...
    function showError(field, message) {
      ...
      const wrapper = field.closest('.form-group, .form-field') || field.parentNode;
    ...
    function validateField(field) {
      if (field.disabled || field.type === 'submit' || field.type === 'button') return true;
      ...
      if (field.hasAttribute('required') && !validators.required(value)) {
        isValid = false;
        errorType = 'required';
      } else if (field.type === 'email' && value.trim() !== '' && !validators.email(value)) {
        isValid = false;
        errorType = 'email';
      } else if (field.type === 'tel' && !validators.tel(value)) {
        isValid = false;
        errorType = 'tel';
      } else if (
        field.hasAttribute('minlength') &&
        value.trim() !== '' &&
        !validators.minlength(value, field.getAttribute('minlength'))
      ) {
        isValid = false;
        errorType = 'minlength';
      }
      ...
      if (!isValid) {
        showError(field, getErrorMessage(field, errorType));
      } else {
        clearError(field);
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "ABUCHI משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '66bbca40-87b9-4eb1-a7c8-c197ed6e5998',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '66bbca40-87b9-4eb1-a7c8-c197ed6e5998';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '66bbca40-87b9-4eb1-a7c8-c197ed6e5998';
